...
from filter_main_page import write_html
from filter_main_page import get_index_html,remove_tags_with_prohibited_words
from filter_main_page import import_file_to_list,get_save_path,transfer_link_to_filename
...
def save_searched_url(url):
    ...
    with open(filename, 'a', encoding='utf-8') as file:
        if url not in existing_words:
            file.write(url + '\n')
            logging.info(f"url '{url}' 已添加。")
            # print(f"url '{url}' 已添加。")
        else:
            logging.info(f"url'{url}' 已存在，未添加。")
    # 可选：确认网址写入成功
...
def run_gui():
    ...
    def get_base_url_and_bid_words():  #获取网址并赋值给 global_base_url
        # try:
        global global_test_url,proxies
        proxy_port=entry0.get()
        if not proxy_port:
            # 如果是，则设置代理为None
            proxies = None
        else:
            # 如果不是空字符串，可以进一步处理代理端口号
            # proxies = f"localhost:{proxy_port}"
            proxies = {
        ...
        if global_test_url and forbidden_words :
            ...
            logging.info("您输入的网址是: %s", global_base_url)
            logging.info("要过滤的违禁词: %s", forbidden_words)
            # print("您输入的网址是：", global_base_url)
            # print("要过滤的违禁词:", forbidden_words)
            runs()
            # root.destroy()
        else:
            logging.info("网址/违禁词为空,请检查后重新输入")
    ...
    label0 = tk.Label(root, text="请输入您的代理端口号（不输入默认不使用代理）：")
    label0.place(relx=0.5, rely=0.1, anchor=tk.CENTER)  # 放在高度的10%位置

    label1 = tk.Label(root, text="请输入您要访问的完整网址：")
    label1.place(relx=0.5, rely=0.3, anchor=tk.CENTER)  # 调整到高度的30%位置

    label2 = tk.Label(root, text="请输入要过滤的违禁词(按空格间隔)：")
    label2.place(relx=0.5, rely=0.5, anchor=tk.CENTER)  # 调整到高度的50%位置

    entry0 = tk.Entry(root, width=40)  # 设置输入框的宽度
    entry0.place(relx=0.5, rely=0.15, anchor=tk.CENTER)  # 调整到高度的15%位置

    entry = tk.Entry(root, width=40)  # 设置输入框的宽度
    entry.place(relx=0.5, rely=0.35, anchor=tk.CENTER)  # 调整到高度的35%位置

    entry2 = tk.Entry(root, width=40)  # 设置输入框的宽度
    entry2.place(relx=0.5, rely=0.55, anchor=tk.CENTER)  # 调整到高度的55%位置

    select_file = tk.Button(root, text="选择", command=import_file)
    select_file.place(relx=0.9, rely=0.55, anchor=tk.CENTER)
...
def runs():
    ...
    if not flag:  # 未关闭
        ...
        get_index_html(global_base_url,page_save_path,proxies)  # 爬取网页
        ...
        logging.info("程序运行时长：%.2f 秒", duration)
